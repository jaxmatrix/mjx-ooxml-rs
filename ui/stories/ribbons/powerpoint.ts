 ...
 * `Shell/PowerPoint` draws Picture Tools.
 *
 * **Chart Tools' Format is authored**, the sixth and last: seven groups and twenty-two commands, which part of a chart
 * on a slide is selected and how to format or reset it, a shape over the chart, how the part is styled, how its text is
 * dressed, and how the chart is described, placed and sized. It is Word's tab through Word's functions (the census's
 * `chartCurrentSelectionCommands`, `insertShapesCommands` for a chart, `shapeStylesCommands`, `wordArtStylesCommands`,
 * `chartFormatAccessibilityCommands`, `arrangeCommands` and `sizeCommands`, and the lists in
 * `stories/ribbons/chart-tools-menus.ts`), and differs where Office does: no Position or Wrap Text, an Eyedropper under
 * all four colour pickers, a chart with no fill and no outline, a slide's starting measures and Size's *Size and
 * Position* launcher. **`Ribbons/PowerPoint` alone binds it**, because `Shell/PowerPoint` draws Picture Tools. No
 * contextual tab is a placeholder any more.
 *
 ...
 * PowerPoint's sixth contextual tab authored and the sixteenth of all, in **Office's** order, which is also the census's.
 * It sits under the *Chart Tools* band, and its accessible name, *Format, Chart Tools*, is what tells it from Shape
 * Format.
 *
 * ⚠ **A contextual tab: Office shows it only while a chart on a slide is selected.** `Ribbons/PowerPoint` draws every
 * contextual set and binds it; **`Shell/PowerPoint` draws Picture Tools alone**, so it binds none of this tab and renders
 * none of its menus. `dev/ribbons/census.ts` records every disagreement.
 *
 * **Word's Chart Format less Position and Wrap Text, under PowerPoint's ids.** Eighteen of the tab's twenty-two commands
 * are bound by the host: the Chart Elements field; Shapes, Change Shape, Shape Effects, Text Effects, Align, Group and
 * Rotate, dropdowns; Bring Forward and Send Backward, split buttons; the Theme Styles and Quick Styles galleries, the
 * first with Other Theme Fills under it; Shape Fill, Shape Outline, Text Fill and Text Outline, colour pickers, each with
 * an Eyedropper; Height and Width, measure fields. Every menu is in `stories/ribbons/chart-tools-menus.ts`. Format
 * Selection and Reset to Match Style are plain buttons, and Alt Text and Selection Pane generic toggles.
 *
 * **Three dialog launchers**: Format Shape on Shape Styles, Format Text Effects on WordArt Styles, **Size and Position**
 * on Size, where Word's says Layout. **No survivor.**
 */
export function powerpointChartFormatTab(options: TabOptions = {}): TemplateResult {
  ...
  return tab(
    ...
    censusGroup(chartFormat, 'GroupSize', { launcher: 'Size and Position' }, controls),
  );
}

/**
 * Which function builds which contextual tab. **All six are authored**: Table Design, Table Layout, Picture Format,
 * Shape Format, Chart Design and Chart Tools' Format — see Word's.
 */
const contextualBuilders: Readonly<Record<string, (options: TabOptions) => TemplateResult>> = {
  ...
  'chart-format': powerpointChartFormatTab,
};
