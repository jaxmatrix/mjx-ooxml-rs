...
export const iconRequests: readonly IconRequest[] = [
  ...
  },
  {
    ...
    sizes: [20, 24],
    variants: ['regular'],
    why: 'Get a Link, on the Share group — the command that produces a URL rather than sending a file, and the chain is what distinguishes the two. 24 since unit 3: **Link** on the Insert tab Links group is large in PowerPoint and Excel, and small in Word. **Since Word\'s Shape Format unit, also Create Link** in the Text group, small, which flows a text box into the next: a chain is what it makes, and it reads *hyperlink* first, so it is that tab\'s weakest glyph. `link-multiple` was the alternative and reads as *several links*. `GUESS:`.',
  },
  {
    name: 'presenter',
    sizes: [20, 24],
  ...
  },
  {
    ...
    sizes: [20, 24],
    variants: ['regular'],
    why: "Text Direction on PowerPoint's Paragraph group and Orientation on Excel's Alignment group — the same command in two vocabularies, and Fluent's rotated glyph is what both applications draw. **Since Word's Table Layout unit, also Text Direction** in Alignment, large, which turns the selected cells' text a quarter further with each press: the same command, so the same glyph, and 24 too. `GUESS:`. **Since PowerPoint's Table Layout unit, also its Text Direction**, a large dropdown over Horizontal, Rotate all text 90°, Rotate all text 270° and Stacked. **Since Word's Shape Format unit, also Text Direction in Word's Shape Format Text group**, small, which rotates a text box's text rather than a cell's. `GUESS:`.",
  },
  {
    name: 'align-center-vertical',
    sizes: [20],
    variants: ['regular', 'filled'],
    why: "Align Text on PowerPoint's Paragraph group and Middle Align on Excel's Alignment group. Vertical alignment, which every application in this unit has and none of them names the same way. `filled` is for Middle Align, which is a toggle; Align Text opens a menu and draws `regular` alone. **Since Word's Shape Format unit, also Align Text in Word's Shape Format Text group**, small and not a toggle, the same command for a text box: Top, Middle, Bottom.",
  },
  {
    name: 'align-top',
    sizes: [20],
  ...
  },
  {
    ...
    sizes: [20, 24],
    variants: ['regular'],
    why: "Shapes, the first command of PowerPoint's Drawing group. Three overlapping outlines, which is what the gallery underneath it contains. 24 since unit 3, where Shapes is a `size=\"large\"` command of the Insert tab's Illustrations group in all three applications. **Since PowerPoint's Shape Format unit, also Insert Shapes' Shapes**, large, standing where Office draws its in-ribbon shape gallery, and opening the same whole gallery Insert's opens. **Since Word's Shape Format unit, also Word's Insert Shapes' Shapes**, the same large dropdown. **Since Excel's Shape Format unit, also Excel's**, the same large dropdown over Excel's copy of the gallery. **Since Word's Chart Format unit, also a chart's Insert Shapes' Shapes**, the same large dropdown over the gallery less New Drawing Canvas and Action Buttons. `GUESS:`. **Since PowerPoint's Chart Format unit, also PowerPoint's**, over PowerPoint's gallery less Action Buttons. `GUESS:`. **Since Excel's Chart Format unit, also Excel's**, over Excel's gallery, which has neither. `GUESS:`.",
  },
  {
    name: 'layer',
    sizes: [20],
