...
Thirty runnable programs. Every one but `mjx-xml`'s `mjx248_measure` — the serialization
...
cargo run -p mjx-ooxml --example guide_saving_validates    # the guide's tri-language examples,
cargo run -p mjx-ooxml --example guide_the_round_trip       # each also a Python and a Node test
