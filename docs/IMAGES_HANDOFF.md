...
```
Also `add_image` (parts only, for `FillSpec::Picture`), `picture_image_rel_id`, `picture_image_bytes`,
...
- **One shape index space over every kind.** `slide::shapes` yields all six `EG_ShapeElements`
  ...
  `p:sp`.
- **`p:blipFill` is built in `mjx-pptx`**, not by `mjx_dml::PictureFill::new` (that emits `a:blipFill`);
  reading it back does reuse `PictureFill`, whose fidelity wrapper is name-agnostic.
...
slide → image relationship, return the id to hand to `FillSpec::Picture`. **A shape can now be filled
...
- **`mjx-dml::PictureFill` already models the picture fill** — `PictureFill::new(interner, rel_id, mode)`,
  `image_rel_id()`, `image_link_id()`, `mode()`, plus interner-free `FillSpec::Picture { rel_id, mode }`
  and `PictureFillMode::{Tile, Stretch, None}`. **No `mjx-dml` changes were needed for I1.**
...
`nav::namespace_prefix`, or match by local name as `PictureFill::image_rel_id` already does); no
