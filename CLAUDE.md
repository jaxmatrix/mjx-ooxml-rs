...
- **`unsafe_code = "deny"`** workspace-wide; a crate that truly needs it must `#[allow(unsafe_code)]`
  locally **with a written safety justification**. Exactly two crates do —
  `bindings/mjx-python` and `bindings/mjx-wasm` — and in both the justification is the same: *no
  hand-written `unsafe`; every unsafe block is generated by `#[pyclass]` / `#[wasm_bindgen]`.* CI
  greps `bindings/*/src` for `unsafe` outside that comment and fails if it finds any, so the
