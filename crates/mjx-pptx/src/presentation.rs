...
use mjx_chart::{
    ...
    ChartSpace, DanglingPointReference, DataLabelSettings, DataLabelSpec, EmbeddedWorkbook,
    ErrorBarDirection, ErrorBarSpec, ErrorBarType, ErrorValueType, LegendPosition, Series,
    TickLabelPosition, TickMark, TrendlineKind, TrendlineSpec,
};
use mjx_dml::{
...
impl Presentation {
    ...
    fn remove_hyperlink_rel_if_unreferenced(
        &mut self,
        part: &PartName,
        rel_id: &str,
    ) -> Result<(), PptxError> {
    ...
    fn retarget_image_relationship(
        &mut self,
        surface: Surface,
        rel_id: &str,
        bytes: &[u8],
    ...
    fn next_part_in(&self, dir: &str, stem: &str, extension: &str) -> Result<PartName, PptxError> {
    ...
    /// [`ChartDataError::DecorationNotAllowed`] (a surface plot has no `c:dLbls`),
    /// [`ChartDataError::DataPointOutOfRange`] (the point does not exist) or
    /// [`ChartDataError::SettingNotAtThisTier`] (leader lines on one point's label).
    ...
    /// [`ChartDataError::TrendlineOrderOutOfRange`] / [`ChartDataError::TrendlinePeriodOutOfRange`].
    ...
    /// [`ChartDataError::CustomErrorBarsNeedValues`].
...
fn build_nv_graphic_frame_pr(interner: &mut Interner, id: u32, name: &str) -> RawElement {
    let cnvpr_attrs = vec![
...
fn chart_series_at(space: &ChartSpace, series_idx: usize) -> Result<&Series, PptxError> {
    let count = space.series_count();
    space
        .plot_area()
        .and_then(|area| area.all_series().nth(series_idx))
