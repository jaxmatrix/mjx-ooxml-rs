//! Shared helpers for `docx_operations` (MJXOFF-147).
//!
...
//! On this crate's 20,000-paragraph corpus, "find element 10,000" pre-order visits the first
//! ~10,000: a mutable search would turn a one-attribute edit into one that, for span-preservation
//! purposes, touched half the document — silently turning a `save_lightly_edited` benchmark into a
//! second `save_fully_materialized` one, and folding an O(n) search into what is supposed to
