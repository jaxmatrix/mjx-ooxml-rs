...
| Validate a `.vml` **as a whole** against a schema | Its root is a bare `<xml>` in **no namespace**, which the VML schemas declare no global element for, so there is nothing for a validator to start from. Every *child* of that wrapper is validated on its own against `vml-main.xsd` (MJXOFF-245), which applies that child's own sequence as well as its elements and attributes. What stays unchecked is the order of the wrapper's own children — and `<xml>` is declared by no schema in either pinned tree, so it has no content model to be out of (MJXOFF-264) |
