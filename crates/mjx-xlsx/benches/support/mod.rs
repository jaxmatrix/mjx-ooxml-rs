//! Shared helpers for `xlsx_operations` (MJXOFF-147).
//!
...
//! On this crate's ~610,000-element corpus, finding the middle attributed cell pre-order visits
//! roughly the first 150,000: a mutable search would turn a one-attribute edit into one that, for
//! span-preservation purposes, touched a quarter of the document — silently turning a
//! `save_lightly_edited` benchmark into something much closer to a `save_fully_materialized` one,
//! and folding an O(n) search into what is supposed to
//! isolate the O(1) cost of mutating an attribute you already have a path to. So
