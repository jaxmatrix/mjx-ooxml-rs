...
pub fn run() -> Result<()> {
    ...
    // 3b. PresentationML simple types (same allowlist discipline as DrawingML — pml.xsd holds the
    //     whole animation/timing vocabulary, so only the curated types are emitted).
    let pml_xsd_path = transitional_dir.join("pml.xsd");
    let pml_xsd = std::fs::read(&pml_xsd_path)
        .with_context(|| format!("reading {}", pml_xsd_path.display()))?;
    let presentationml_src = emit::emit_selected_types(
        &pml_xsd,
        "References/.../OfficeOpenXML-XMLSchema-Transitional/pml.xsd",
        emit::PRESENTATIONML_MODULE_DOC,
        PRESENTATIONML_TYPES,
    )?;
    write_generated(&out_dir.join("presentationml.rs"), &presentationml_src)?;
    ...
    println!(
        "codegen: wrote child_order.rs, drawingml.rs, presentationml.rs, namespaces.rs, \
         shared.rs, mod.rs, COVERAGE.md"
    );
    Ok(())
}
...
fn coverage_manifest(shared_xsd: &[u8]) -> Result<String> {
    ...
    s.push_str(") |\n");
    for schema in ["wml", "sml", "shared-math"] {
    ...
    for schema in ["dml-main", "pml", "dml-chart"] {
        let status = if CHILD_ORDER_SCHEMAS.contains(&schema) {
            "every complex type"
        } else {
            "pending"
        };
        s.push_str(&format!("| {schema} | {status} |\n"));
    }
    for schema in ["wml", "sml", "shared-math"] {
        s.push_str(&format!("| {schema} | pending |\n"));
    }
