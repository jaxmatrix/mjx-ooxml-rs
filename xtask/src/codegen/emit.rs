...
/// The `@generated` banner, the regeneration command and the source note every emitted module
/// opens with, followed by that module's own `//!` block.
///
/// `pub` because the nine `*_MODULE_DOC` constants above are public and name it, and a public doc
/// comment may not link to a private item under CI's `RUSTDOCFLAGS`.
/// `xtask/tests/codegen_drift.rs` asserts that every committed generated file still opens with the
/// banner this writes.
pub fn file_header(source_note: &str, module_doc: &str) -> String {
    format!(
