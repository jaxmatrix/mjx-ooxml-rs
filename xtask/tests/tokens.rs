...
/// The four artefacts, relative to the root they are written under.
///
/// `derivations.css` joined them in MJXOFF-271: it is the derived tier restated as the
/// `color-mix()` expressions it came from, so that a host's seed override re-themes the chrome
/// through the cascade with no code.
...
fn the_committed_artefacts_are_exactly_what_the_source_generates() {
    ...
    assert!(
        reported.contains("4 artefacts match"),
        "`--check` must say what it checked, so a run that checked nothing is visible: {reported}"
    );
}

/// The four artefacts are committed, so a checkout has to contain them. A `--check` that passed
/// because it compared two empty strings would be exactly the failure this ticket names.
#[test]
fn every_artefact_is_committed_and_carries_its_generated_header() {
    ...
    for (relative, contents) in read_artefacts(&root) {
        ...
        // The derivation layer names the scheme-relative aliases rather than the flat palette, so
        // it is asked for the thing it is actually made of instead of for a token it never
        // mentions — a shared assertion that a file can only satisfy vacuously is not one.
        let landmark = if relative.ends_with("derivations.css") {
            "color-mix(in srgb, "
        } else {
            "--color-green"
        };
        assert!(
            contents.contains(landmark),
            "{relative} does not mention `{landmark}`"
        );
    }
}
