...
fn declared_members() -> Vec<String> {
    ...
    let members: Vec<String> = members
        .split('"')
        // ⚠ **Only the odd fragments are inside quotes**, and the parity check is load-bearing
        // rather than tidiness. Splitting on `"` alternates between outside and inside; without
        // this, a *comment* between two entries that happens to contain a `/` is read as a member.
        // The client platform's comments do exactly that — they name generated artefacts by path —
        // so this walk reported a paragraph of prose as a crate with no `lib.rs`.
        .enumerate()
        .filter_map(|(index, fragment)| (index % 2 == 1).then_some(fragment))
