...
binding surface exercised by its own suite:
  Python        996 of 1640 declared member(s) (60.7%), 644 named by no test
  WebAssembly   933 of 1764 declared member(s) (52.9%), 831 named by no test
```
...
* *Exercised* is an **upper bound**. The matcher works on names, so a `.rows` anywhere credits
  `rows` on every class that declares one. The true figure is lower than 996 and lower than 933.
...
`every_javascript_name_is_the_camel_case_of_its_rust_name` holds all 1,764 exported functions to the
