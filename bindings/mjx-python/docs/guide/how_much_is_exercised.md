...
* *Exercised* is an **upper bound**. The matcher works on names, so a `.rows` anywhere credits
  `rows` on every class that declares one. The true figure is lower than 1,012 and lower than 946.
* *Not exercised* is **exact**. No test can call a member whose name appears nowhere in any test
  source. 632 and 822 are floors on the untested surface, not estimates of it.
...
`every_javascript_name_is_the_camel_case_of_its_rust_name` holds all 1,768 exported functions to the
