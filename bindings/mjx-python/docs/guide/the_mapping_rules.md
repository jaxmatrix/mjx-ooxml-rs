...
The rules above say where a *name* comes from. A name has a sentence beside it, and it has a source
too — a different one in each language, which is the whole of what is worth knowing here.

**In Python the `///` doc comment on the `#[pymethods]` item *is* the docstring.** PyO3 compiles it
verbatim into the member's `__doc__`, so `help(mjx_ooxml.Deck.save)` prints the comment written in
`bindings/mjx-python/src/deck.rs`. There is no second audience: a comment there that talks about
Rust — a `[`name`](Type::method)` intra-doc link, or "see this module's own doc comment" — is prose
about something a Python reader cannot open, and it is wrong in `help()` before it is wrong
anywhere else.

**The committed stub does not write its own.** Until MJXOFF-234 it did, and the two copies drifted:
`bindings/mjx-python/tests/test_stub_parity.py` compares *names* in both directions and
`test_every_deck_method_carries_a_docstring` asks only that a docstring exists, so the sentence
beside each name was unchecked text. MJXOFF-226 found the two disagreeing about the same item and
differently wrong in each — the signature of two hand-maintained copies of one fact. Now
`bindings/mjx-python/tools/stub_docs.py` copies each `__doc__` into the stub and
`bindings/mjx-python/tests/test_stub_docs.py` fails if the committed file is not what it writes.
**Editing a docstring in `python/mjx_ooxml/__init__.pyi` is a test failure**: change the Rust
comment and re-run the tool.

```sh
python bindings/mjx-python/tools/stub_docs.py            # restate the committed stub's docstrings
python bindings/mjx-python/tools/stub_docs.py --check    # write nothing; report whether it is current
```

Only prose is generated. The signatures are still written by hand, and
`bindings/mjx-python/tests/test_stub_parity.py` is still what holds them to the module.

**TypeScript's sentence has a different source, and the two *are* compared.** `mjx_ooxml.d.ts` is
generated by `wasm-bindgen` from the `///` comments in `bindings/mjx-wasm/src/`, so it cannot drift
from them either — but those comments are written independently of the Python binding's, so **a
TypeScript reader and a Python reader could be told different things about the same method**.
`xtask/tests/binding_doc_parity.rs` (MJXOFF-266) is what stops that. It lives in `xtask` because the
two bindings are siblings and neither may see the other, and it reads the two source trees as text.

Equality cannot be the gate, and that is the whole difficulty: a sentence naming `chart_series` must
say `chartSeries` in TypeScript, one naming `str` must say `string`, and `None` must be `undefined`.
So the comparison **normalises first** — every identifier inside a backtick span is written
`snake_case`, a closed table folds the language words onto one spelling, and articles are dropped —
and two members are a pair only when they also **take the same number of arguments**, because where
the signatures differ the projection differs and the prose is supposed to differ with it. Whatever
still differs is on that file's `DIVERGENT` ledger with a written reason, held to the measurement in
both directions: a new divergence fails until somebody writes a reason, and a row whose sentences
have since been unified fails until somebody deletes it. The test prints today's pair counts rather
than this page quoting them.

Building it found twenty-three members where a TypeScript reader was told strictly less than a
Python one — six `CellFormatSpec.applies*` getters that never mentioned `undefined`, `Document.open`
with no error paragraph at all, `FontProperties.scheme` documented as the single word `scheme` — and
they were fixed rather than recorded.

## Nothing else differs

That is the whole list of **name and shape** differences — the prose is the section above —
and it is checkable rather than asserted. The two bindings export the same
