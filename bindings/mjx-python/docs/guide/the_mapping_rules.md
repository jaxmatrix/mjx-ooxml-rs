...
**TypeScript's sentence has a different source, and the two are not compared.** `mjx_ooxml.d.ts` is
...
TypeScript reader and a Python reader can be told different things about the same method**. They
usually are not: of the 1,610 members both bindings document under the same class and Rust name,
1,440 carry the same sentence character for character. The 170 that differ are not all drift — a
sentence naming `chart_series` must say `chartSeries` in TypeScript, and one naming `str` must say
`string` — which is exactly why equality cannot be the gate here and why this one is **not closed**.
It is tracked as MJXOFF-266.
