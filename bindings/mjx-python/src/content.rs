...
impl LayoutInfo {
    ...
    #[getter]
    fn kind(&self) -> PyResult<SlideLayoutKind> {
...
impl PlaceholderInfo {
    ...
    #[getter]
    fn kind(&self) -> PyResult<PlaceholderType> {
    ...
    #[getter]
    fn size(&self) -> PyResult<PlaceholderSize> {
        PlaceholderSize::from_model(self.0.size)
    }

    /// Horizontal or vertical.
    #[getter]
    fn orientation(&self) -> PyResult<Orientation> {
...
impl OleObject {
    ...
    #[getter]
    fn prog_id(&self) -> Option<&str> {
        self.0.prog_id.as_deref()
    }
...
impl InkReference {
    ...
    #[getter]
    fn shape_index(&self) -> Option<u32> {
        self.0.shape_index
    }
...
impl DiagramContent {
    ...
    #[staticmethod]
    fn from_parts(data: Vec<u8>, layout: Vec<u8>, style: Vec<u8>, colors: Vec<u8>) -> Self {
    ...
    #[staticmethod]
    fn vertical_list(labels: Vec<String>) -> Self {
    ...
    #[getter]
    fn data<'py>(&self, python: Python<'py>) -> Bound<'py, PyBytes> {
        PyBytes::new(python, &self.0.data)
    }

    /// The layout definition part's bytes.
    #[getter]
    fn layout<'py>(&self, python: Python<'py>) -> Bound<'py, PyBytes> {
        PyBytes::new(python, &self.0.layout)
    }

    /// The style definition part's bytes.
    #[getter]
    fn style<'py>(&self, python: Python<'py>) -> Bound<'py, PyBytes> {
        PyBytes::new(python, &self.0.style)
    }

    /// The colour transform part's bytes.
    #[getter]
    fn colors<'py>(&self, python: Python<'py>) -> Bound<'py, PyBytes> {
        PyBytes::new(python, &self.0.colors)
    }
...
impl Hyperlink {
    ...
    fn kind(&self) -> &'static str {
        match &self.0 {
            ooxml::Hyperlink::Url(_) => "url",
            ooxml::Hyperlink::Slide(_) => "slide",
        }
    }
    ...
    fn target(&self) -> Option<&str> {
        match &self.0 {
            ooxml::Hyperlink::Url(url) => Some(url),
    ...
    #[getter]
    fn slide_index(&self) -> Option<u32> {
        match &self.0 {
...
impl OleObjectData {
    /// A raw stream, embedded in the package.
    #[staticmethod]
    fn embedded_stream(bytes: Vec<u8>) -> Self {
    ...
    /// `.pptx`, say.
    #[staticmethod]
    fn embedded_package(bytes: Vec<u8>, extension: &str, content_type: &str) -> Self {
    ...
    fn kind(&self) -> &'static str {
        match &self.0 {
            OwnedOleData::EmbeddedStream(_) => "embedded_stream",
            OwnedOleData::EmbeddedPackage { .. } => "embedded_package",
            OwnedOleData::Linked(_) => "linked",
        }
    }
...
impl OleObjectData {
    ...
    fn borrowed(&self) -> ooxml::OleObjectData<'_> {
        match &self.0 {
...
impl OleObjectSpec {
    /// An OLE object.
    #[new]
    #[pyo3(signature = (prog_id, data, snapshot_image, name = None, show_as_icon = false))]
    fn new(
        prog_id: &str,
        data: OleObjectData,
        snapshot_image: Vec<u8>,
    ...
    #[staticmethod]
    fn embedded_stream(prog_id: &str, data: Vec<u8>, snapshot_image: Vec<u8>) -> Self {
        Self {
            prog_id: prog_id.to_owned(),
    ...
    fn shown_as_icon(&self, show_as_icon: bool) -> Self {
        let mut spec = self.clone();
    ...
    #[getter]
    fn data(&self) -> OleObjectData {
        self.data.clone()
    }

    /// The picture shown in the object's place.
    #[getter]
    fn snapshot_image<'py>(&self, python: Python<'py>) -> Bound<'py, PyBytes> {
        PyBytes::new(python, &self.snapshot_image)
    }
...
impl ActiveXControlSpec {
    ...
    #[getter]
    fn persistence(&self) -> ActiveXPersistence {
        self.persistence
    }

    /// The persisted state, when there is any.
    #[getter]
    fn state<'py>(&self, python: Python<'py>) -> Option<Bound<'py, PyBytes>> {
        self.state.as_ref().map(|state| PyBytes::new(python, state))
    }
...
#[pyfunction]
#[pyo3(name = "default_placeholder_audio")]
fn default_placeholder_audio(python: Python<'_>) -> Bound<'_, PyBytes> {
    PyBytes::new(python, &ooxml::default_placeholder_audio())
}

/// A one-frame, zero-length MP4 — a video placeholder that every consumer accepts.
#[pyfunction]
#[pyo3(name = "default_placeholder_video")]
fn default_placeholder_video(python: Python<'_>) -> Bound<'_, PyBytes> {
    PyBytes::new(python, &ooxml::default_placeholder_video())
}

/// An empty compound file — an OLE object placeholder that every consumer accepts.
#[pyfunction]
#[pyo3(name = "default_placeholder_ole")]
fn default_placeholder_ole(python: Python<'_>) -> Bound<'_, PyBytes> {
    PyBytes::new(python, &ooxml::default_placeholder_ole())
}

/// Adds every class and function in this module to the extension module.
pub(crate) fn register(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add_class::<LayoutInfo>()?;
    module.add_class::<ShapeInfo>()?;
    module.add_class::<PlaceholderInfo>()?;
    module.add_class::<MediaReference>()?;
    module.add_class::<LinkedImage>()?;
    module.add_class::<OleObject>()?;
    module.add_class::<ExternalLink>()?;
    module.add_class::<InkReference>()?;
    module.add_class::<DiagramParts>()?;
    module.add_class::<DiagramRelationshipIds>()?;
    module.add_class::<DiagramContent>()?;
    module.add_class::<Hyperlink>()?;
    module.add_class::<OleObjectData>()?;
    module.add_class::<OleObjectSpec>()?;
    module.add_class::<ActiveXControlSpec>()?;
    module.add_function(wrap_pyfunction!(default_placeholder_audio, module)?)?;
    module.add_function(wrap_pyfunction!(default_placeholder_video, module)?)?;
    module.add_function(wrap_pyfunction!(default_placeholder_ole, module)?)
}
