...
//! `wasm-bindgen` emits them into the `.d.ts`, so a consumer gets completion and a compile error for
...
macro_rules! sealed_enums {
    ($(
        ...
        $(
            ...
            impl $name {
                /// The model's value as this enumeration's member. Never fails: the upstream
                /// enumeration is closed and every variant is named above.
                pub fn from_model(value: ooxml::$name) -> Result<Self, JsValue> {
...
macro_rules! open_enums {
    ($(
        ...
        $(
            ...
            impl $name {
                /// The model's value as this enumeration's member, or an `OoxmlError` with code
                /// `UnsupportedContent` if the model has grown a variant this build does not
                /// project.
                pub fn from_model(value: ooxml::$name) -> Result<Self, JsValue> {
                    Ok(match value {
